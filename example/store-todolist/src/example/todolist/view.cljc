...
;; Experimental utility stuff
(defn- for-fragment-map [^vd/ReactiveRef map-ref child-component]
  (vw/for-fragment* (fn [] @map-ref)
                    first
                    (fn [[k ^vd/IViewData v]]
                      ;;(pprint [:map-entry k v @v])
                      ($ child-component k (vd/reactive-ref v)))))

;; Experimental utility stuff
(defn- for-fragment-vector [^vd/ReactiveRef vector-ref child-component]
  (vw/for-fragment* (fn [] (->> @vector-ref (map-indexed vector)))
                    second
                    (fn [[index ^vd/IViewData item]]
                      ;;(pprint [:vector-entry index item @item])
                      ($ child-component index (vd/reactive-ref item)))))
